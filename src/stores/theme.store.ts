...
export const useThemeStore = defineStore('theme', () => {
  ...
  const canSave = computed(
    () => state.value.isDirty && editingPreset.value && !editingPreset.value.metadata.isBuiltIn,
  )
  ...
  async function initialize() {
    ...
    try {
      ...
      const preset =
        presets.value.find((p) => p.id === savedPresetId) ||
        presets.value.find((p) => p.metadata.isDefault)
  ...
  function setConfigVisible(visible: boolean) {
    console.log('🎯 setConfigVisible called', {
      visible,
      currentState: state.value.configSidebarVisible,
    })
