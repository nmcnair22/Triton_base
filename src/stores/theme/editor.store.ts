...
export const useThemeEditorStore = defineStore('theme-editor', () => {
  ...
  const hasChanges = computed(() => {
    ...
    const colorsChanged = JSON.stringify(editingColors.value) !== JSON.stringify(originalColors.value)
    
    // Check if preset properties have changed
    const presetChanged = editingPreset.value.name !== presetStore.activePreset.name ||
  ...
  const canSave = computed(() => 
    hasChanges.value && 
    editingPreset.value && 
    !editingPreset.value.metadata.isBuiltIn
  )
  
  const isEditingBuiltIn = computed(() => 
    editingPreset.value?.metadata.isBuiltIn || false
  )
  ...
  async function updateColor(tokenId: string, value: string) {
    ...
    if (existingIndex >= 0) {
      ...
      if (shouldHavePalette(tokenId)) {
        try {
          const { generatePaletteMemoized } = await import('@/utils/performance/color-utils-optimized')
  ...
  function getTokenValue(tokenId: string): string {
    ...
    if (presetStore.activePreset) {
      const presetOverride = presetStore.activePreset.colorOverrides.find(c => c.tokenId === tokenId)
  ...
  watch(() => presetStore.activePreset, (newPreset) => {
    if (newPreset) {
      startEditing(newPreset)
    }
  }, { immediate: true })
